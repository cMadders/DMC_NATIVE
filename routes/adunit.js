...
router.get('/:adunitID/:index?', function(req, res, next) {
  ...
  AdUnit.find(
    ...
    function(err, adunit) {
      ...
      var embed = `(function(data) { var placeholder = document.currentScript; if (!window.DMC) { window.DMC = { dependenciesLoaded: false, adunits: [], activate: function(cb) { for (var i = 0; i < window.DMC.adunits.length; i++) { var unit = window.DMC.adunits[i]; if (!unit.activated) { return unit.activate(i, cb) || null; } } } }; } window.DMC.adunits.push({ data: data, activated: false, activate: function(index, cb) { this.activated = true; cb(index, this.data, placeholder); } }); var loadDependencies = function() { var dependencies = ['//cdnjs.cloudflare.com/ajax/libs/jquery/3.3.1/jquery.slim.min.js', '//cdnjs.cloudflare.com/ajax/libs/gsap/1.18.2/plugins/CSSPlugin.min.js', '//cdnjs.cloudflare.com/ajax/libs/gsap/1.18.2/TweenLite.min.js',${card_js}]; var head = document.head || document.getElementsByTagName('head')[0]; for (var i = 0; i < dependencies.length; i++) { var s = document.createElement('script'); s.async = false; s.type = 'text/javascript'; s.src = dependencies[i]; head.appendChild(s); } }; if (!window.DMC.dependenciesLoaded) { loadDependencies(); } })('${adunit_data}');`
