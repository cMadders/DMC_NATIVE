...
function getNativeListings(adunitID, cb) {
  AdUnit.findById(adunitID, { creatives_to_persist: 1, creatives: 1 }, function(
    err,
    adunit
  ) {
  ...
  async.waterfall(
    [
      function(cb) {
        AdUnit.findById(
          adunitID,
          {
            creatives_to_persist: 1,
            creatives: 1,
            'extra.dmc_publication_id': 1
          },
          function(err, adunit) {
            if (err) return cb(err)
      ...
      function(publicationID, cb) {
        fetchDMCPublication(
          { adunitID: adunitID, publicationID: publicationID },
          cb
        )
      },
      // PROCESS
    ...
    function(err, result) {
      ...
      res.json({
        ...
        count: 'Creatives Synced: ' + result.length
      })
    }
  )
})
...
router.post('/publication', function(req, res, next) {
  if (req.body && req.body.publicationID) {
    async.waterfall([async.apply(fetchDMCPublication, req.body)], function(
      err,
      result
    ) {
      res.json(result)
...
router.post('/creative/update', function(req, res, next) {
  ...
  req.body['extra.protect_overwrite'] =
    req.body['extra.protect_overwrite'] === undefined ? false : true
  async.waterfall(
    [
      // update ad unit
      function(callback) {
...
router.get('/creatives-removed/:adunitID', function(req, res, next) {
  ...
  CreativeRemoved.find({ adunit_id: req.params.adunitID }, function(
    err,
    adunit
  ) {
    if (err) return handleError(err)
...
router.post('/adunit/update', function(req, res, next) {
  ...
  req.body['extra.dmc_xul_auto_sync'] =
    req.body['extra.dmc_xul_auto_sync'] === undefined ? false : true

  if (
    !req.body['extra.dmc_publication_id'] ||
    !req.body['extra.dmc_publication_key']
  ) {
...
function fetchDMCListing(req, type, cb) {
  ...
  request(url, function(err, response, body) {
    if (err) {
      ...
      if (body.length == 2) {
        console.log(
          'XUL listing not found: ' + req.listingID + ' | type: ' + type
        )
        return cb(
          null,
          {
...
function writeCreative(creative, req, cb) {
  ...
  Creative.findOne({ 'extra.listingID': listingID }, function(
    err,
    existingCreative
  ) {
    if (err) {
      ...
      if (existingCreative) {
        ...
        AdUnit.findOne({ _id: req.adunitID }, function(err, adunit) {
          ...
          if (adunit && adunit.creatives) {
            ...
            var exists = _.findWhere(adunit.creatives, {
              ...
              AdUnit.findById(req.adunitID, function(err, adunit) {
                ...
                adunit.save(function(err) {
                  ...
                  return cb(null, {
                    response: 'success',
                    message:
                      'creative exists - association with ad unit now complete',
                    req: req,
                    creative: existingCreative
                  })
                })
